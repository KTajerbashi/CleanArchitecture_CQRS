...
using CleanArchitectureCQRS.Application.Library.Resolvers;
...
namespace CleanArchitectureCQRS.ContextDatabase.Library.Base.Infrastructure;
...
public class AggregateRepository<TA, TKey> : IAggregateRepository<TA, TKey> where TA : class, IAggregateRoot<TKey>
{
    ...
    public async Task AppendAsync(TA aggregate, CancellationToken cancellationToken = default)
    {
        ...
        var result = await _eventStoreClient.AppendToStreamAsync(streamName, StreamState.Any, eventList.ToArray(), cancellationToken: cancellationToken);
    }
